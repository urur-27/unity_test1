...
public class Chat : MonoBehaviour
{
	...
	void SelectHostTypeGUI()
	{
		...
		if (GUI.Button(new Rect(0, 0, 200, 30), "채팅방 만들기"))
		{
		...
		if (GUI.Button(new Rect(0, 30, 200, 30), "채팅방 들어가기"))
		{
	...
	void ChattingGUI()
	{
		...
		if (GUI.Button(new Rect(350, 500, 80, 30), "나가기"))
		{
	...
	public void OnEventHandling(NetEventState state)
	{
		switch (state.type)
		{
			case NetEventType.Connect:
				if (m_transport.IsServer())
				{
					AddMessage(ref m_message[1], "상대방이 입장했습니다.");
				}
				else
				{
					AddMessage(ref m_message[0], "상대방과 이야기할 수 있습니다.");
				}
				break;
			...
			case NetEventType.Disconnect:
				if (m_transport.IsServer())
				{
					AddMessage(ref m_message[0], "상대방이 나갔습니다.");
				}
				else
				{
					AddMessage(ref m_message[1], "상대방이 나갔습니다.");
				}
				break;
		}
	}
